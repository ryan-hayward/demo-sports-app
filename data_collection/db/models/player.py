...
from sqlalchemy import Column, String, Integer, Date, ARRAY, JSON

# add root directory of project to path and get api gateway
sys.path.insert(1, "/".join(os.path.realpath(__file__).split("/")[0:-3]))
# import local module for sending HTTP requests and scraping data
...
class Player(Base):
    ...
    positions = Column("positions", ARRAY(String))
    throws = Column("throws", String)
    height = Column("height", Integer)
    weight = Column("weight", Integer)
    team = Column("team", String)
    dob = Column("dob", Date)
    birth_region = Column("birth_region", String)
    college = Column("college", String)
    draft = Column("draft", JSON)

    # note: not assigning position right away as multiple can be added
    def __init__(self, player_id: str, player_name: str, position: str, gateway: ApiGateway):
        # utils
